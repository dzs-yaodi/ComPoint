package com.xw.customrecycler.gallery;
...
public class RecyclerGallery extends RecyclerView {
     ...
     * 布局器构建者
     */
    private GalleryLayoutManager.Builder mManagerBuilder;

    public RecyclerGallery(@NonNull Context context) {
        super(context);
        init();
    }

    public RecyclerGallery(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public RecyclerGallery(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void createManageBuilder() {
        if (mManagerBuilder == null) {
            mManagerBuilder = new GalleryLayoutManager.Builder();
        }
    }
    ...
    protected int getChildDrawingOrder(int childCount, int i) {
        int center = getGalleryManager().getCenterPosition()
                - getGalleryManager().getFirstVisiblePosition(); //计算正在显示的所有Item的中间位置
     ...
     */
    public GalleryLayoutManager getGalleryManager() {
        return (GalleryLayoutManager) getLayoutManager();
    }
    ...
    public int getSelectedPos() {
        return getGalleryManager().getSelectedPos();
    }
     ...
     * @param l 监听接口
     */
    public void setOnItemSelectedListener(GalleryLayoutManager.OnSelected l) {
        getGalleryManager().setOnSelectedListener(l);
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if ((ev.getX() > mDownX && getGalleryManager().getCenterPosition() == 0) ||
                        (ev.getX() < mDownX && getGalleryManager().getCenterPosition() ==
                                getGalleryManager().getItemCount() -1)) {
